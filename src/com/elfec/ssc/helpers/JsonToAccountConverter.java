...
/**
 * Clase que sirve para convertir un JSONObject a una cuenta con su información completa
 * @author drodriguez
 *
 */
	...
	public static Account convert(JSONObject accObj)
	{
		try {
				Account account = new Account(Client.getActiveClient(), 
						accObj.getString("AccountNumber"), 
						accObj.getString("NUS"), 
				...
				for (int i = 0; i < lenght; i++) 
				{
					...
					{
						account.getDebts().add(new Debt(
								account, 
								new BigDecimal(debtObj.getString("Amount").replace(',', '.')), 
								debtObj.getInt("Year"), 
								Short.parseShort(debtObj.getString("Month")), 
								debtObj.getInt("ReceiptNumber"), 
								DateTimeFormat.forPattern("dd/MM/yy")
								.parseDateTime(debtObj.getString("ExpirationDate"))));					}
				}
				return account;
			} 
